...
export function renderNextSteps(intent: DisplayIntent): string {
  const steps = (intent.next_steps || [])
    .map((s) => {
      const { label, action, hidden, query } = typeof s === "string" ? { label: s, action: s, hidden: false, query: undefined } : s;
      const hiddenAttr = hidden ? ' data-hidden="1"' : "";
      // A `query` step carries the exact {tool, args} for the next call —
      // no reasoning needed, so the frontend can run it directly against
      // /turn/next-page without ever involving the LLM. `data-action`
      // stays populated as a fallback for any plugin build that doesn't
      // recognize `data-query` yet.
      const queryAttr = query ? ` data-query="${escapeHtml(JSON.stringify(query))}"` : "";
      const cls = query ? "erp-agent-next-step erp-agent-next-page" : "erp-agent-next-step";
      return `<button class="${cls}" data-action="${escapeHtml(action)}"${queryAttr}${hiddenAttr}>${escapeHtml(label)}</button>`;
    })
    .join("");
