 ...
 */
// 2026-08-23: this_quarter/last_quarter/last_year added per explicit
// user request ("add anything missing") — every existing value kept
// (day_before_yesterday and the last_N_days values are real fixes for
// confirmed-live 2026-08-12 bugs, dropping them would reopen those),
// the 3 new ones given real resolveRelativePeriod() cases + phrase
// patterns below, not just listed here with nothing behind them (that
// would throw "Unknown relative period" the first time the model
// actually picked one).
export const RELATIVE_PERIODS = [
  "today", "yesterday", "day_before_yesterday", "this_week", "last_week", "this_month", "last_month",
  "last_7_days", "last_14_days", "last_30_days", "last_60_days", "this_year", "this_quarter",
...
export function resolveRelativePeriod(rawPeriod: string, todayIso: string): [string, string] {
  ...
  switch (period as RelativePeriod) {
    ...
    // Calendar-quarter-to-date, same "calendar-aligned, not a trailing
    // window" convention this_month/this_week already use — Q1=Jan-Mar,
    // Q2=Apr-Jun, Q3=Jul-Sep, Q4=Oct-Dec.
    case "this_quarter": {
      const qStartMonth = Math.floor((m - 1) / 3) * 3;
      return [fmt(new Date(Date.UTC(y, qStartMonth, 1))), todayIso];
    }
    // Full previous calendar quarter (start to end) — same "full period,
    // not a trailing window" convention last_month already uses.
    // Date.UTC() normalizes a negative month index itself (e.g. month -3
    // in Q1 correctly rolls back to October of the PRIOR year), so no
    // separate year-rollover branch is needed.
    case "last_quarter": {
      const qStartMonth = Math.floor((m - 1) / 3) * 3;
      const thisQStart = new Date(Date.UTC(y, qStartMonth, 1));
      const lastQEnd = addDays(thisQStart, -1);
      const lastQStart = new Date(Date.UTC(y, qStartMonth - 3, 1));
      return [fmt(lastQStart), fmt(lastQEnd)];
    }
    // Full previous calendar year — same "full period" convention as
    // last_month/last_quarter, not a trailing 365 days.
    case "last_year":
...
/**
 * Real, live-found gap (2026-08-26): a genuine "monthly trend over the
 * last 6 months" question has no way to express itself through the
 * fixed single-period vocabulary above at all — RELATIVE_PERIODS has
 * nothing between "last_month" (one month back) and "last_60_days" (a
 * trailing window, not calendar-month-aligned, and still only ~2
 * months) — there is no "3/4/5/6 months ago" a caller could even ask
 * for. Confirmed live: asked for a 6-month invoice trend, the model
 * had no real way to construct it, rambled about "adjusting the date
 * range", and eventually fabricated a mislabeled result instead.
 *
 * Same "never ask the model to do date arithmetic itself" philosophy
 * as resolveRelativePeriod above — this returns N real, calendar-
 * aligned month buckets (oldest first) in ONE deterministic call,
 * so a caller never needs to compute "3 months ago" itself at all.
 * The most recent entry is the CURRENT month, capped at todayIso (it
 * may be partial); every entry before that is a full completed
 * calendar month.
 */
export function resolveLastNMonths(todayIso: string, n: number): { label: string; start: string; end: string }[] {
  const [y, m] = todayIso.split("-").map(Number);
  const months: { label: string; start: string; end: string }[] = [];
  for (let i = n - 1; i >= 0; i--) {
    const monthIndex = m - 1 - i; // 0-based; Date.UTC normalizes a negative index into the correct prior year
    const first = new Date(Date.UTC(y, monthIndex, 1));
    const lastDayOfMonth = new Date(Date.UTC(y, monthIndex + 1, 0)).toISOString().slice(0, 10);
    months.push({
      label: first.toLocaleString("en-US", { month: "long", year: "numeric", timeZone: "UTC" }),
      start: first.toISOString().slice(0, 10),
      end: i === 0 ? todayIso : lastDayOfMonth, // current month capped at today, since it may be partial
    });
  }
  return months;
}
