...
router.post("/logout", requireAuth, (req: AuthedRequest, res) => {
  ...
  // Frees this session's chat memory immediately rather than leaving it
  // to sit in memory until it would've expired anyway — see
  // sessionCacheProvider.ts's per-session (not per-user) keying.
  sessionCacheProvider.clear(req.sessionId!);
  res.json({ ok: true });
});
