 ...
 */
// Sample only, commented out — not wired in, no dependency installed.
// Shows the shape a real SMTP-backed send() would take (nodemailer),
// so this stays illustrative rather than a live connection. See the
// class doc comment above for full "go live" instructions.
//
...
// });
//
// async function realSend(userEmail: string, mail: { to: string; subject: string; body: string }) {
//   await transporter.sendMail({ from: process.env.SMTP_FROM, to: mail.to, subject: mail.subject, text: mail.body });
//   return sentEmailStore.push(userEmail, mail); // still record it, same as the stub
// }
