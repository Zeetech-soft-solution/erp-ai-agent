...
// "opportunity" is used throughout (not sales_invoice/quotation — those
// aren't part of this tier's entity map) — it has both a real numeric
// field (amount -> opportunity_amount) and a real date field
// (date -> transaction_date), enough to exercise count()/aggregate()'s
// generic chunking logic same as any other entity would.
...
describe("ErpNextConnector.count", () => {
  ...
  it("calls frappe.client.get_count with the real doctype and native filters, zero rows fetched", async () => {
    ...
    const result = await connector.count("opportunity", credential, { status: "Open" });
    expect(callMethod).toHaveBeenCalledWith(
      "frappe.client.get_count",
      { doctype: "Opportunity", filters: JSON.stringify([["status", "=", "Open"]]) },
      expect.anything()
    );
  ...
  it("coerces a string count (some RPC paths return numeric strings) to a real number", async () => {
    ...
    expect(await connector.count("opportunity", credential)).toBe(42);
  });

  it("omits filters entirely when none were given", async () => {
    callMethod.mockResolvedValue(10);
    await connector.count("opportunity", credential);
...
describe("ErpNextConnector.aggregate — op:count shortcuts to count(), no row fetch", () => {
  ...
  it("bare count with no groupBy never calls getDocList, at any population size", async () => {
    ...
    const result = await connector.aggregate("opportunity", credential, { op: "count" });
    expect(result).toEqual({ overall: { value: 58000, count: 58000 } });
    expect(getDocList).not.toHaveBeenCalled();
  });
});

describe("ErpNextConnector.aggregate — under the cap, single fetch-and-reduce", () => {
  const connector = new ErpNextConnector();
  const credential = { mode: "api_key", apiKey: "k", apiSecret: "s" } as any;
  ...
  beforeEach(() => {
    ...
    getDocList.mockResolvedValue([{ name: "A", opportunity_amount: 100 }, { name: "B", opportunity_amount: 250 }, { name: "C", opportunity_amount: 50 }]);
  });

  it("sums correctly and only fetches once", async () => {
    const result = await connector.aggregate("opportunity", credential, { field: "amount", op: "sum" });
  ...
  it("computes a real weighted average, not a naive one", async () => {
    const result = await connector.aggregate("opportunity", credential, { field: "amount", op: "avg" });
    expect(result.overall.value).toBeCloseTo(400 / 3);
  });

  it("min/max read the real extremes", async () => {
    const min = await connector.aggregate("opportunity", credential, { field: "amount", op: "min" });
    const max = await connector.aggregate("opportunity", credential, { field: "amount", op: "max" });
...
describe("ErpNextConnector.aggregate — over the cap WITH a date-range filter, chunks and combines exactly", () => {
  ...
  function dateRangeFrom(paramsFilters: string | undefined): [string, string] | null {
    ...
    const dateTriple = triples.find((t: any) => t[0] === "transaction_date");
  ...
  beforeEach(() => {
    ...
    getDocList.mockImplementation(async (_doctype: string, params: any) => {
      ...
      if (start === "2024-01-01") return [{ name: "H1a", opportunity_amount: 1000 }, { name: "H1b", opportunity_amount: 2000 }];
      return [{ name: "H2a", opportunity_amount: 500 }];
    });
  });
  ...
  it("sums both halves together into one exact total", async () => {
    const result = await connector.aggregate("opportunity", credential, { field: "amount", op: "sum", filters });
  ...
  it("computes a real cross-chunk weighted average, not an average of the two chunk averages", async () => {
    const result = await connector.aggregate("opportunity", credential, { field: "amount", op: "avg", filters });
  ...
  it("min/max span across both chunks correctly", async () => {
    const min = await connector.aggregate("opportunity", credential, { field: "amount", op: "min", filters });
    const max = await connector.aggregate("opportunity", credential, { field: "amount", op: "max", filters });
  ...
  it("never fetches more than the two under-cap leaf chunks (no full-range fetch attempted)", async () => {
    await connector.aggregate("opportunity", credential, { field: "amount", op: "sum", filters });
...
describe("ErpNextConnector.aggregate — over the cap with NO date filter falls back to a capped, warned fetch", () => {
  ...
  beforeEach(() => {
    ...
    getDocList.mockResolvedValue([{ name: "A", opportunity_amount: 100 }]);
    warnSpy = jest.spyOn(console, "warn").mockImplementation(() => {});
  });
  afterEach(() => warnSpy.mockRestore());

  it("still returns a (possibly partial) number rather than throwing, and warns about it", async () => {
    const result = await connector.aggregate("opportunity", credential, { field: "amount", op: "sum", filters: { status: "Open" } });
...
describe("ErpNextConnector.aggregate — groupBy merges the same group key across chunks", () => {
  ...
  beforeEach(() => {
    ...
    getDocList.mockImplementation(async (_doctype: string, params: any) => {
      ...
      // Same party ("Acme") appears in BOTH halves — real-world shape:
      // one customer with opportunities spanning both years.
      if (start === "2024-01-01") return [{ name: "H1", opportunity_amount: 100, party_name: "Acme" }];
      return [{ name: "H2", opportunity_amount: 300, party_name: "Acme" }, { name: "H3", opportunity_amount: 50, party_name: "Beta" }];
    });
  });

  it("combines the SAME group's partial totals across chunks instead of overwriting", async () => {
    const result = await connector.aggregate("opportunity", credential, {
      field: "amount",
      op: "sum",
      groupBy: "party",
...
describe("ErpNextConnector.list — default page size comes from settingsService, not a hardcoded 100", () => {
  ...
  it("uses the admin-configured default when the caller didn't pass an explicit limit", async () => {
    ...
    await connector.list("opportunity", credential, {});
  ...
  it("an explicit caller-supplied limit still wins over the default", async () => {
    settingsService.get.mockResolvedValue(25);
    await connector.list("opportunity", credential, { limit: 200 });
    const [, params] = getDocList.mock.calls[0];
