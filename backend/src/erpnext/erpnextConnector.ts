...
import { DOCUMENT_LINK_MAP } from "./documentLinkMap";
...
export class ErpNextConnector implements SystemConnector {
  ...
  async list(entityKey: string, credential: UserCredential, params?: { filters?: Record<string, any>; limit?: number; offset?: number; sortBy?: string; sortDir?: "asc" | "desc" }): Promise<any[]> {
    ...
    const canonicalRows = rows.map((row: any) => toCanonicalRow(entityKey, row));
    // One extra query PER LINK FIELD for this whole page, batched — not
    // per row. Safe even at report.generate's up-to-10,000-row scale;
    // see backfillDocumentLinks' own doc comment for why this is
    // deliberately NOT the same one-lookup-per-record pattern
    // backfillPrimaryContactInfo/backfillEmployeePhone use.
    if (DOCUMENT_LINK_MAP[entityKey]) {
      await this.backfillDocumentLinks(entityKey, canonicalRows, credential);
    }
  ...
  /**
   * DOCUMENT_LINK_MAP's real work — see that file's own doc comment for
   * the full verification story. Real, hard-won mechanism (three prior
   * approaches tried and disproven with live evidence before this one,
   * see git history / project memory for the full trail — permission
   * grant, parenttype filter, and service-credential all failed or were
   * unnecessary): query the PARENT doctype's own get_list with a DOTTED
   * child-field reference (e.g. "items.prevdoc_docname") — real Frappe
   * ORM support, confirmed against both raw Python (frappe.get_list) and
   * the actual REST endpoint this code calls. This is why the ACTING
   * USER'S OWN credential works here — permission is checked against the
   * parent doctype, which they already demonstrably have, never the
   * child doctype independently (which, confirmed live, silently strips
   * every requested field to just "name" for ANY caller including
   * Administrator — a genuine Frappe platform limitation for istable
   * doctypes queried directly, not a permission gap at all).
   *
   * Still batched — one query PER LINK FIELD for the whole page/batch,
   * not one per row, same reasoning as before (safe even at
   * report.generate's up-to-10,000-row scale).
   *
   * One real wrinkle from the dotted-field join: it returns ONE ROW PER
   * CHILD ITEM, not one per parent (a real SQL join, not a per-document
   * summary) — a Sales Order with 3 line items comes back as 3 rows,
   * potentially with the SAME source value repeated (confirmed live:
   * every item on a real Sales Order pointed at the same source
   * Quotation) or, in principle, different ones per line. "First non-
   * empty value wins" is the correct, simple choice for the common case;
   * a genuinely mixed-source parent would just show its first item's
   * source, not corrupt or crash — an acceptable simplification, not a
   * silent-wrong-answer.
   */
  private async backfillDocumentLinks(entityKey: string, canonicalRows: Record<string, any>[], credential: UserCredential): Promise<void> {
    const mappings = DOCUMENT_LINK_MAP[entityKey];
    if (!mappings?.length || !canonicalRows.length) return;
    const parentIds = canonicalRows.map((r) => r.id).filter(Boolean);
    if (!parentIds.length) return;
    const parentDoctype = ERPNEXT_ENTITY_MAP[entityKey]?.doctype;
    if (!parentDoctype) return;
    const client = this.clientFor(credential, undefined, { read: true });
    for (const mapping of mappings) {
      try {
        const rows = await getDocList(
          parentDoctype,
          {
            fields: JSON.stringify(["name", `${mapping.parentChildField}.${mapping.nativeLinkField}`]),
            filters: JSON.stringify([["name", "in", parentIds]]),
            limit_page_length: 0, // every matching child row for this batch, not paged — this IS the point of batching
          },
          client
        );
        const byParent = new Map<string, string>();
        for (const row of rows) {
          const value = row[mapping.nativeLinkField];
          if (value && !byParent.has(row.name)) byParent.set(row.name, value); // first non-empty wins, see doc comment above
        }
        for (const row of canonicalRows) {
          const value = byParent.get(row.id);
          if (value) row[mapping.canonicalField] = value;
        }
      } catch (err) {
        // A document-link backfill failing (permissions, a doctype
  ...
  async get(entityKey: string, credential: UserCredential, id: string): Promise<any> {
    ...
    if (DOCUMENT_LINK_MAP[entityKey]) {
      await this.backfillDocumentLinks(entityKey, [canonical], credential);
    }
    return canonical;
  }
  ...
  // (Draft -> Submitted, docstatus 0 -> 1) — previously had NO backing
  // tool anywhere in this codebase; an autoflow "Submit" next-step would
  // have been pure decoration. Frappe's own REST resource API treats
  // docstatus as a plain settable field on a PUT — the exact same
  // updateDoc() helper update() already uses, no new HTTP plumbing.
  // Setting docstatus:1 runs the real submit validation/workflow
  // server-side (mandatory fields, submit-time hooks, etc.) — a
  // malformed/incomplete doc still fails the same way clicking "Submit"
  // in the real ERPNext desk UI would, this isn't a bypass of anything.
  async submit(entityKey: string, credential: UserCredential, id: string): Promise<any> {
    const mapping = ERPNEXT_ENTITY_MAP[entityKey];
    const row = await updateDoc(mapping.doctype, id, { docstatus: 1 }, this.clientFor(credential));
    return toCanonicalRow(entityKey, row);
  }
  ...
  /** Records a payment against a real Sales Invoice AND submits it — see
   *  this method's own doc comment on SystemConnector for why this leans
   *  on ERPNext's own get_payment_entry() rather than hand-assembling a
   *  Payment Entry from flat fields. Three real API calls, all AS the
   *  acting user (never a service credential, same discipline as every
   *  other write in this file): (1) the whitelisted get_payment_entry
   *  method derives the correct receiving account, currency, and
   *  reference allocation from the invoice itself — the exact same call
   *  ERPNext's own "Create > Payment" desk button makes; (2) that
   *  pre-populated document gets inserted for real; (3) it gets
   *  submitted (docstatus 0 -> 1), which is what actually posts the real
   *  GL entries and reduces the invoice's real outstanding_amount —
   *  deliberately DIFFERENT from every other *.create tool in this app
   *  (which stay Draft), per explicit product decision 2026-08-17: the
   *  accountant's own confirmed chat instruction (this tool's
   *  ruleAction:"create" already requires an explicit confirm-before-
   *  create round trip — see businessRuleEngine.ts) IS the human review
   *  step for this specific action; a second, separate desk-UI review
   *  afterward would be redundant friction, not extra safety. Verified
   *  live on a real small entry before this was wired in: correct GL
   *  accounts, exact invoice balance reduction, balanced debit/credit. */
  async createPaymentEntryForInvoice(credential: UserCredential, invoiceId: string, amount?: number): Promise<any> {
    const client = this.clientFor(credential);
    const body: Record<string, any> = { dt: "Sales Invoice", dn: invoiceId };
    // Confirmed live 2026-08-17: calling this as a GET with amount in the
    // query string crashed inside ERPNext's own code with "bad operand
    // type for abs(): 'str'" — an HTTP query string is always text, and
    // this specific whitelisted method does real arithmetic (abs()) on
    // party_amount without casting it first. A POST with a JSON body
    // preserves the actual number type, which fixes this without needing
    // to touch (or guess at a workaround for) ERPNext's own code.
    if (amount != null) body.party_amount = amount;
    const derived = await client.post("/api/method/erpnext.accounts.doctype.payment_entry.payment_entry.get_payment_entry", body);
    const paymentEntryDoc = derived.data?.message;
    if (!paymentEntryDoc) throw new Error(`ERPNext did not return a payment entry template for invoice "${invoiceId}"`);
    // Confirmed live 2026-08-17: get_payment_entry() derives the receiving
    // account/currency/allocation correctly but does NOT fill reference_no/
    // reference_date — ERPNext's own real-time validation rejects an
    // insert without them whenever the derived receiving account is a bank
    // account (only a plain Cash account is exempt). erpdatabuild's own
    // historical simulation (business_day_engine.py's make_payment_for_invoice)
    // hits this exact same requirement and resolves it with an
    // auto-generated reference tied to the invoice itself, never a fake
    // cheque/transaction number presented as real — same convention here,
    // for consistency with how every other real Payment Entry in this
    // company's data was actually created.
    const today = new Date().toISOString().slice(0, 10);
    const payload = {
      doctype: "Payment Entry",
      ...paymentEntryDoc,
      reference_no: paymentEntryDoc.reference_no || `REF-${invoiceId}`,
      reference_date: paymentEntryDoc.reference_date || today,
    };
    const created = await createDoc("Payment Entry", payload, client);
    // See this method's own doc comment above for why this submits
    // (docstatus 0 -> 1) rather than leaving it Draft like every other
    // *.create tool — this is what actually posts the GL entries and
    // reduces the invoice's real outstanding_amount.
    const submitted = await updateDoc("Payment Entry", created.name, { docstatus: 1 }, client);
    return toCanonicalRow("payment_entry", submitted);
  }

  /** Sends a REAL reply email against an existing Communication thread —
   *  reuses frappe.core.doctype.communication.email.make(), the exact
   *  same whitelisted function ERPNext's own desk "Reply" button calls
   *  (confirmed against the live container's own source, not guessed).
   *  Deliberately NOT a hand-assembled frappe.sendmail() call: make()
   *  already handles real threading (in_reply_to), its own permission
   *  check on the referenced document (frappe.has_permission(doctype,
   *  doc=name, ptype="email", throw=True) — a real, different check from
   *  plain "read"), and optional PDF-attachment-by-print-format in one
   *  proven call. Runs as the acting user's own credential, same
   *  discipline as every other write here — never a service account. */
  async replyToCommunication(credential: UserCredential, params: { communicationId: string; replyBody: string; attachPrintFormat?: string }): Promise<any> {
    const client = this.clientFor(credential);
    const original = await getDoc("Communication", params.communicationId, client);
    if (!original) throw new Error(`No such email "${params.communicationId}" to reply to`);
    const subject = (original.subject as string) || "";
    const body: Record<string, any> = {
      doctype: original.reference_doctype || undefined,
      name: original.reference_name || undefined,
    ...
    if (params.attachPrintFormat && original.reference_doctype && original.reference_name) {
      body.print_format = params.attachPrintFormat;
    }
    const res = await client.post("/api/method/frappe.core.doctype.communication.email.make", body);
    return res.data?.message ?? res.data;
  }

  /** Same real frappe.core.doctype.communication.email.make() as
   *  replyToCommunication above, but with no original thread to reply
   *  to/carry an in_reply_to from — a genuinely fresh outbound email.
   *  Deliberately no doctype/name linking or PDF attachment yet (a real,
   *  separate follow-up), just a plain to/subject/body send. */
  async sendCommunication(credential: UserCredential, params: { recipients: string; subject: string; body: string }): Promise<any> {
    const client = this.clientFor(credential);
    const body: Record<string, any> = {
      content: params.body,
  ...
  /** Marks one Notification Log row read, on behalf of the acting user —
   *  reuses frappe.desk.doctype.notification_log.notification_log.
   *  mark_as_read(), the exact function the desk's own bell panel calls.
   *  Deliberately NOT a generic update(): Notification Log's own real
   *  DocPerm grants "All" read/share but no write at all (Frappe routes
