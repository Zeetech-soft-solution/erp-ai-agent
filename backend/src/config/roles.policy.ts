 ...
 *
 * Free tier: one role, granting exactly the single API this tier
 * exposes (quotation.list). A tool being registered by
 * entityModuleFactory or a hand-written module never implies access;
 * it must also be granted here. To add your own role/tool grants as
 * you extend entities.config.ts, follow this exact shape — key is the
 * ERPNext role name (must match exactly, a mismatch here silently
 * grants nothing), value is the list of tool names to allow.
 */
