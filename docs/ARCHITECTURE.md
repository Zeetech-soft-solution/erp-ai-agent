...
Hand-written modules (`modules/analytics`, `modules/chart`, `modules/schema`,
`modules/dataServer`, `modules/reports`, `modules/inboxActions`,
`modules/paymentEntry`, `modules/documents`, `modules/email`,
`modules/tickets`, `modules/projects`, `modules/toolDiscovery`) follow the
identical rule: they call `systemConnector.<method>(...)`, never ERPNext's
client directly — same discipline as the generic entity factory. Everything
that is a plain list/get/create/update on a real doctype goes through the
generic entity factory instead (one `EntityConfig`, no code) — that is how
all eleven domains get their coverage.
...
```
config/modules/<name>/entity/*.ts     <- one file per canonical entity + an
                                         index.ts barrel
config/modules/<name>/rule/*.ts        <- one RuleSet per entity that has rules
config/modules/<name>/training/index.ts

  names: crm, selling, buying, stock, accounting, hr, manufacturing,
         projects, assets, quality, support

config/entities.config.ts   <- imports + spreads every module's entity/ barrel
config/rules.config.ts      <- imports + spreads every module's rule/ barrel
config/training.config.ts   <- imports + spreads every module's training/
...
**All eleven modules are populated** with canonical entity definitions
covering the standard ERPNext doctypes for each domain, their ERPNext
entity maps, business rules where a domain has real "normal practice" to
enforce (CRM contact/duplicate checks, Selling manager-gated conversion and
pricing-rule range checks, and so on), and training-curation metadata.
Read `config/modules/crm/` or `config/modules/selling/` for the fullest
worked examples of every piece.
...
3. Run every file in `db/migrations/` against your Postgres (with pgvector installed), in
   filename order
4. `npm run dev`
5. Create one ERPNext user with a real role (e.g. **Sales User**) and confirm `/api/auth/login`
   returns a real `allowed_tools` list (see `config/roles.policy.ts` for the map)
6. Test `/api/tools` (list) and `/api/tools/lead.list` (POST) with that token
