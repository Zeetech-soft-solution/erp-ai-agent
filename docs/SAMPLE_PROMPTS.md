...
## Creating and updating — each goes through a real business rule, and a confirm-before-create step

- "Create a lead for Aisha Kapoor, email aisha@example.com" — try it **without** an email
  or phone too; `lead.require_contact_method` blocks it with a clear message instead of
  silently accepting an uncontactable lead.
- "Create a lead for someone at [a company with an existing lead], same email as before" —
  `lead.warn_duplicate_email` flags it (a warning, not a block) rather than silently
